...
class PersonService:
    ...
    async def get_person_by_id(self, person_id: str) -> Optional[Person]:
        person = await self._get_person_by_id_from_cache(person_id)
        if not person:
            person = await self._get_person_by_id_from_elastic(person_id)
            if not person:
                return None
            await self._put_person_by_id_to_cache(person)
        return person

    async def _get_person_by_id_from_cache(self,
                                           person_id: str) -> Optional[Person]:
    ...
    async def _put_person_by_id_to_cache(self, person: Person):
    ...
    async def get_person_list(
        ...
        person_list = await self._get_person_list_from_cache(
            page_number=page_number,
            page_size=page_size)
        if not person_list:
            person_list = await self._get_person_list_from_elastic(
                page_number=page_number,
                page_size=page_size
            )
            if not person_list:
    ...
    async def _get_person_list_from_cache(
            self,
            page_number: int,
            page_size: int) -> Optional[List[Person]]:
    ...
    async def get_person_by_search(self, search_string: str) -> Optional[
        List[Person]]:
        person_list = await self._get_person_by_search_from_cache(search_string)
        if not person_list:
            person_list = await self._get_person_by_search_from_elastic(search_string)
            if not person_list:
                return None
            await self._put_person_by_search_to_cache(search_string,
                                                      person_list)
        return person_list

    async def _get_person_by_search_from_cache(
            self,
            search_string: str
    ...
    async def _get_person_by_search_from_elastic(
            self,
            search_string: str
    ) -> Optional[List[Person]]:
    ...
    async def _put_person_by_search_to_cache(self,
                                             search_string: str,
                                             person_list: List[Person]):
        person_list_json = json.dumps(person_list, default=pydantic_encoder)
    ...
    async def _get_person_films_from_cache(
            self, person_id: str) -> Optional[List[Film]]:
